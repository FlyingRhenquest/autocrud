...
1. `Crud::Read` - Takes an object with its ID set and populates the object if it finds it. It returns `true` if it did something.
1. `Crud::Update` - Updates a record in the database
1. `Crud::Delete` - Deletes a record from the database.
...
1. Create an `Crud` object templated for your structure.
